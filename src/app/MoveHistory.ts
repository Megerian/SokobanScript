import { type LURD_CHAR } from "../Sokoban/PuzzleFormat"
...
export class MoveHistory {
    ...
    getLastDoneMoveDirection(): DIRECTION | null {
        ...
        const lastMoveChar = this.history[this.playedCount - 1] as LURD_CHAR
        return Directions.getDirectionFromLURDChar(lastMoveChar)
    }

    /**
     * Replaces the entire history with the given LURD string.
     * If the string contains a '*' (generated by getSaveGameLURD), it automatically
     * parses the cursor position and reconstructs the undo/redo state.
     *
     * Otherwise, `playedCount` is set to the given value or to the full length if omitted.
     *
    ...
    setHistory(lurd: string, playedCount?: number): void {
        const starIndex = lurd.indexOf("*")

        if (starIndex !== -1) {
            // Safe-game format detected: split and extract cursor
            this.history = lurd.slice(0, starIndex) + lurd.slice(starIndex + 1)
            this.playedCount = starIndex
        } else {
            // Standard raw LURD string
            this.history = lurd
            const targetCount = playedCount !== undefined ? playedCount : lurd.length
            this.playedCount = Math.max(0, Math.min(targetCount, lurd.length))
        }
    }
